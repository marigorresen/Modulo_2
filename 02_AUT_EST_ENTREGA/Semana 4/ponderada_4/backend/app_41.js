...
app.post('/insereCurso', urlencodedParser, (req, res) => {
	...
	res.write('<p>CURSO INSERIDO COM SUCESSO!</p><a href="/">VOLTAR</a>');
	db.close(); // Fecha o banco
	res.end();
});
...
app.post('/atualizaCurso', urlencodedParser, (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); 
	sql = "UPDATE Curso SET Nome_curso='" + req.body.Nome_curso + "', Periodo_curso = '" + req.body.Periodo_curso + "' , Descricao_curso='" + req.body.Descricao_curso + "' WHERE CursoID='" + req.body.CursoID + "'";
	console.log(sql);
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [],  err => {
		if (err) {
...
app.get('/removeCurso', urlencodedParser, (req, res) => {
	...
	db.run(sql, [],  err => {
		...
		res.write('<p>CURSO REMOVIDO COM SUCESSO!</p><a href="/">VOLTAR</a>');
		res.end();
	});
	db.close(); // Fecha o banco
});
